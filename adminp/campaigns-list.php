...
<?php include 'sidebar.php'; ?> 
 ...
 <a href="campaign-details.php" class="btn btn-danger">Add New Campaign</a></h3>      
  <h4 class="sub-title" style="color: chocolate;font-size: 20px;">
Campaigns List:</h4>       
...
<div style="width: 100%;overflow: scroll;">
   ...
   <tbody >  
            <?php
            $qo = "SELECT * FROM campaign_details as c INNER JOIN organizers as b ON c.org_id=b.org_id ORDER BY c.camp_id DESC";
            ...
            while ($res = mysqli_fetch_array($queryh)) {
                ...
                $qot = "SELECT * FROM donars_list WHERE camp_id='$camp_id'";
                ...
                $queryht = mysqli_query($con, $qot);
                while ($rest = mysqli_fetch_array($queryht)) {
                    $w_mainimage=$rest['cimg_url'];
                    break;
                }
               
       ?><tr>
            <td>
            <?php echo "MASS/CP/".date('Y')."/".$camp_id; ?></td>
              <td><?php echo convertdate($res['camp_startdate']);  ?> TO <?php if($res['camp_startdate']!="NA" || $res['camp_startdate']!=null){ echo convertdate($res['camp_enddate']); } ?></td>
          
            <td> 
 <img style="max-width: 90px;" src="<?php echo "../camp-documents/".$w_mainimage; ?>"  alt="<?php echo $res['camp_title']; ?>">
            </td>
            <td>  <?php if($res['camp_status']==2){ ?> <button class="btn btn-danger">Inactive</button>
 <?php } else if($res['camp_status']==1){ ?>  <button class="btn btn-success">Active</button> <?php } ?>
</td>
                 <td><?php echo $res['camp_title']; ?></td>
            <td><?php echo formatCustomNumber($res['camp_target_amount']); ?></td>
             <td><?php echo formatCustomNumber($res['camp_current_amount']); ?></td> 
                        <td><?php echo $tdoners; ?></td> 
                <td><a href="campaign-details.php?camp_id=<?php echo $camp_id; ?>" target="_blank" style="font-weight: 700;color: chocolate;">
                <button>Edit</button></a>
                <a href="../campaigns.php?camp_id=<?php echo $camp_id; ?>&s=<?php echo $camp_slug; ?>" target="_blank" style="font-weight: 700;color: chocolate;">
